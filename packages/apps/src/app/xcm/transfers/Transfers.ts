...
export class Transfers extends BaseApp {
  ...
  private async syncStatus(operations: Map<string, Transfer>) {
    const updated = this.transfers.map(async (t: Transfer) => {
      const info = t.content.info;

      if (info.status === TransferStatus.Completed) {
        return t;
      }

      const o = operations.get(t.id);
      if (o) {
        const isComplete = await this.transferApi.isTransferComplete(
          t,
          o.vaa.raw,
        );
        t.content.info.status = isComplete
          ? TransferStatus.Completed
          : TransferStatus.VaaEmitted;
      } else {
  ...
  emptyTemplate() {
    return html`
      <div slot="empty" class="empty">
        ...
        <span>${i18n.t('transfers.noActivity')}</span>
      </div>
    `;
  }
