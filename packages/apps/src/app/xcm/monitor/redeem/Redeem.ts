...
import { StandartizedOperation } from '../types';
...
import './RedeemDatagrid';

import styles from './Redeem.css';
import { WatchContractEventReturnType } from 'viem';

@customElement('gc-redeem')
export class Redeem extends BaseApp {
  private redeemApi: RedeemApi = null;
  private transferSubs: WatchContractEventReturnType = null;

  @state() operations: StandartizedOperation[] = [];
  @state() width: number = window.innerWidth;
  @state() active: string = 'deposit';
  @state() loading: boolean = true;

  static styles = [baseStyles, headerStyles, basicLayoutStyles, styles];

  private async syncTransfers() {
    this.redeemApi.getOperations().then((operations) => {
  ...
  override connectedCallback() {
    ...
    this.init();
    this.syncTransfers();
    this.transferSubs = this.redeemApi.subscribeRedeem((vaa, tx) => {
      console.log(vaa, tx);
    });
  }
  ...
  headerTemplate() {
    return html`
      <uigc-typography slot="header" class="header" variant="section">
        Wormhole
      </uigc-typography>
    `;
  }
  ...
  tabsTemplate() {
    return html`
      <div slot="tabs" class="tabs">
        ${this.tabTemplate('deposit', 'Deposit')}
        ${this.tabTemplate('withdraw', 'Withdraw')}
      </div>
    `;
  }
  ...
  render() {
    ...
    return html`
      <gc-redeem-grid class="redeem" .defaultData=${data} .showHeader=${true}>
        ${this.headerTemplate()} ${this.tabsTemplate()}
