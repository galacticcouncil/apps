...
export class TradeOrders extends BaseApp {
  ...
  private filters: Record<string, (order: Order) => boolean> = {
    all: (order: Order) => true,
    open: (order: Order) => !order.status,
    finished: (order: Order) => !!order.status,
  };
  ...
  private async syncOrders() {
    ...
    if (this.assets.size > 0) {
      ...
      const orders = scheduled.map(async (order: Order) => {
        const remaining = await this.getRemaining(order.id);
